...
function parseFrontmatter(body: string): { title?: string; description?: string; full?: unknown } | null {
...
export function validateDocsTree(rootInput: string, opts: ValidateOptions = {}): ValidateResult {
  const errors: string[] = [];
  ...
  for (const p of files) {
    ...
    if (st.size > FILE_BYTES) errors.push(`file too large (${st.size}): ${rel}`);
    total += st.size;
  ...
  try {
    ...
    return { ok: false, errors: [...errors, "missing or invalid docs/meta.json"] };
  }
  ...
  if (!Array.isArray(meta.pages)) {
    ...
    for (const entry of meta.pages) {
      ...
      if (!pageExists(root, entry)) errors.push(`meta.json pages missing: ${entry}`);
    }
  }
  ...
  for (const p of files) {
    ...
    if (!fm.description) errors.push(`frontmatter description required: ${rel}`);
    ...
    for (const m of stripped.matchAll(/<([A-Z][A-Za-z0-9]*)\b/g)) {
      const tag = m[1];
      if (!ALLOWED_COMPONENTS.has(tag)) errors.push(`unknown MDX tag <${tag}>: ${rel}`);
    }
    for (const m of stripped.matchAll(/<include\b[^>]*>([^<]*)<\/include>/gi)) {
      ...
      if (!isInside(root, real)) errors.push(`include escapes tree: ${rel} -> ${target}`);
    }
  }

  if (opts.markdownlintConfig) {
    const mdx = files.filter((p) => !lstatSync(p).isSymbolicLink() && extname(p) === ".mdx");
    if (mdx.length > 0) {
      const bin = resolve(import.meta.dir, "..", "node_modules", ".bin", "markdownlint-cli2");
      const r = spawnSync(bin, ["--config", opts.markdownlintConfig, ...mdx], { encoding: "utf8" });
      if (r.status !== 0) {
...
if (import.meta.main) {
  ...
  const config = process.argv.includes("--lint")
    ? resolve(import.meta.dir, "..", "content", "docs", ".markdownlint.jsonc")
    : undefined;
